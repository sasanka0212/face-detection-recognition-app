...
class FaceAlignerService {
  ...
  img.Image alignFace({
    ...
    required FaceLandmarks landmarks,
  }) {

    final xs =
        landmarks.points.map((e) => e.x);

    final ys =
        landmarks.points.map((e) => e.y);
    ...
    final cropX =
        max(0, (minX - paddingX).toInt());

    final cropY =
        max(0, (minY - paddingY).toInt());

    final cropW =
        min(
          image.width - cropX,
          (width + paddingX * 2).toInt(),
        );

    final cropH =
        min(
          image.height - cropY,
          (height + paddingY * 2).toInt(),
        );
